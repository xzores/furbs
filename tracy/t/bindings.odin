...
when ODIN_OS == .Darwin  { 
	foreign import tracy "TracyClient.dylib"
}
when ODIN_OS == .Windows {
	//@(extra_linker_flags="/DEFAULTLIB:libcmt.lib /NODEFAULTLIB:msvcrt.lib")
	foreign import tracy "TracyClient.lib"
}
when ODIN_OS == .Linux   { 
	foreign import tracy "TracyClient.so"
}
...
foreign tracy {
	...
	___tracy_alloc_srcloc                               :: proc( line: u32, source: cstring, sourceSz: c.size_t, function: cstring, functionSz: c.size_t, color : [4]u8) -> u64 ---
	___tracy_alloc_srcloc_name                          :: proc( line: u32, source: cstring, sourceSz: c.size_t, function: cstring, functionSz: c.size_t, name: cstring, nameSz: c.size_t, color : [4]u8) -> u64 ---
