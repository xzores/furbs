...
draw_shape :: proc(shape : Shape, rot : f32 = 0, texture : Maybe(Texture2D) = nil, shader := gui_shader, color : [4]f32 = {1,1,1,1}, loc := #caller_location) {
	...
	place_uniform(shader, builtin_uniforms.col_diffuse, color);
	
	if tex_diffuse, ok := texture.?; ok {
		place_uniform(shader, builtin_uniforms.texture_diffuse, tex_diffuse);
	}
	else {
		place_uniform(shader, builtin_uniforms.texture_diffuse, get_white_texture());
	}
