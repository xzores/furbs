...
Shader :: struct {
	...
	name : string,
	attribute_locations : map[Attribute_client_index]Attribute_info, 	// Shader locations array (MAX_SHADER_LOCATIONS)
	uniform_locations : map[Uniform_client_index]Uniform_info,
}
...
load_shader :: proc(using shader : ^Shader, vs_name : string, fs_name : string, loc := #caller_location) {
	...
	for k, &e in shader.attribute_locations {
		e.location = -1;
	}

	for k, &e in shader.uniform_locations {
		e.location = -1;
	}

	attrib_names := make(map[string]Attribute_client_index);
	defer delete(attrib_names);

	uniform_names := make(map[string]Uniform_client_index);
	defer delete(uniform_names);

	attrib_enums := reflect.enum_fields_zipped(user_enum_attribute_locations);
	for enum_field, i in attrib_enums { 
		name := enum_field.name;
		value : Attribute_client_index = auto_cast enum_field.value;
		attrib_names[name] = value;
	}

	//Load uniforms into uniform_names
	uniform_enums := reflect.enum_fields_zipped(user_enum_uniform_locations);
	for enum_field, i in uniform_enums {
		name := enum_field.name;
		value : Uniform_client_index = auto_cast enum_field.value;
...
place_uniform :: proc(shader : Shader, uniform_loc : $U, value : $T, loc := #caller_location) where intrinsics.type_is_enum(U) || U == Uniform_client_index {
	...
	assert(U == uniform_enum_type, "The uniform passed is not the same used to initialize furbs", loc = loc);

	uniform_info : Uniform_info;
	
	if T == Uniform_client_index {
		uniform_info = shader.uniform_locations[auto_cast uniform_loc];
	}
	else if T == uniform_enum_type {
		uniform_info = shader.uniform_locations[auto_cast uniform_loc];
	}
	...
	if auto_cast uniform_loc in texture_locations {
		//it is a texture
...
bind_shader :: proc(shader : Shader, loc := #caller_location) {
	...
	place_uniform(shader, builtin_uniforms.prj_mat, prj_mat);
	place_uniform(shader, builtin_uniforms.inv_prj_mat, inv_prj_mat);

	place_uniform(shader, builtin_uniforms.view_mat, view_mat);
	place_uniform(shader, builtin_uniforms.inv_view_mat, inv_view_mat);
