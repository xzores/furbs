...
destroy_ECS :: proc (ecs : ^ECS) {
	
}

//msg_size in bytes
register_message_id :: proc (ecs : ^ECS, name : string, msg_size : int) -> Message_id {
	ecs.cur_msg_id += 1;
	return ecs.cur_msg_id;
}

//Does not take any ownership of the data
register_system :: proc (ecs : ^ECS, name : string, size_of_component : int, msg_subs : map[Message_id]proc(components : []rawptr, origin : Entity_id, msg_data : rawptr), requirements : []Component_id) -> Component_id {
...
make_entity :: proc (ecs : ^ECS) -> Entity_id {
	ecs.cur_entity_id += 1;
...
destroy_entity :: proc (ecs : ^ECS, entity : Entity_id) {
	panic("TODO");
}

add_component :: proc (ecs : ^ECS, entity_id : Entity_id, component_type : Component_id, component_data : rawptr, data_size : int) {
...
//use for update
trigger_msg_all :: proc (ecs : ^ECS, msg_type : Message_id, msg_data : rawptr) {
...
add_messages :: proc (ecs : ^ECS, msg : Message) {
