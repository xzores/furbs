...
when RENDER_DEBUG {
	...
	Living_state :: struct {
		programs	: map[Shader_program_id]Object_debug_info,
		buffers		: map[Buffer_id]Object_debug_info,
		vaos		: map[Vao_id]Object_debug_info,
		fbos		: map[Fbo_id]Object_debug_info,
		tex1ds		: map[Tex1d_id]Object_debug_info,
		tex2ds		: map[Tex2d_id]Object_debug_info,
		tex3ds		: map[Tex3d_id]Object_debug_info,
		tex_cubemaps: map[Tex_cubemap_id]Object_debug_info,
		rbos		: map[Rbo_id]Object_debug_info,
		syncs		: map[gl.sync_t]Object_debug_info,
	}
...
GL_state :: struct {
	...
	active_unit : u32, //which unit is active (used for opengl verions below 4.5)
	bound_tex1ds	: [16]Tex1d_id,
	bound_tex2ds	: [16]Tex2d_id,
	bound_tex3ds	: [16]Tex3d_id,
	bound_tex_cube	: [16]Tex_cubemap_id,
	//TODO other texture locations
...
init :: proc(gl_context := context) {
	...
	when gl.GL_DEBUG {
		if cpu_state.gl_version >= .opengl_4_3 {
			...
			//THIS CAUSES ERRORS IN RENDER DOC gl.DebugMessageCallback(debug_callback, nil);
		}
	}
		...
		for field in reflect.struct_fields_zipped(Living_state) {
			...
			if map_type_info, ok := field.type.variant.(runtime.Type_Info_Map); ok {
				...
				if value_info, ok := map_type_info.value.variant.(runtime.Type_Info_Named); ok {
					fmt.assertf(value_info.name == "Object_debug_info", "Map must hold Object_debug_info, found : %s", value_info.name)
				}
			...
			if key_size == 4 {
				s := cast(^map[u32]Object_debug_info)(cast(uintptr)&debug_state.living + field.offset);
				for id, info in s {
					log.errorf("Leak detected! %v with id %i has not been deleted, but allocated at location : %v", field.name, id, info.loc);
					leaks += 1;
				}
			}
			else if key_size == 8 {
				s := cast(^map[u64]Object_debug_info)(cast(uintptr)&debug_state.living + field.offset);
				for id, info in s {
					log.errorf("Leak detected! %v with id %i has not been deleted, but allocated at location : %v", field.name, id, info.loc);
					leaks += 1;
				}
...
//Uses temp alloc
extend_label :: proc(label : string, concat : string) -> string {
	
	if label == "" {
		return "";
	}
	
	return fmt.tprintf("%v%v", label, concat);
}
...
set_cubemap_seamless :: proc (seamless : bool) {
	if cpu_state.cube_map_seamless == seamless {
		return;
	}
	...
	if seamless {
		...
		gl.Disable(.TEXTURE_CUBE_MAP_SEAMLESS)
	}
	cpu_state.cube_map_seamless = seamless
}
...
@(require_results)
load_shader_program :: proc(name : string, vertex_src : string, fragment_src : string, label : string, loc := #caller_location) -> (Shader_program_id, Maybe(Compilation_error)) {
...
unload_shader_program :: proc(shader : Shader_program_id, loc := #caller_location) {
	...
	when RENDER_DEBUG {
		assert(shader in debug_state.programs, "The shader program is not valid", loc);
		delete(debug_state.programs[shader].label);
...
gen_vertex_arrays :: proc(vaos : []Vao_id, label : string, loc := #caller_location) {
...
gen_vertex_array :: proc(label : string, loc := #caller_location) -> Vao_id {
...
delete_vertex_arrays :: proc (vaos : []Vao_id, loc := #caller_location) {
	...
	when RENDER_DEBUG {
		for vao in vaos {
			assert(vao in debug_state.vaos, "The VAO is not valid", loc);
			delete(debug_state.vaos[vao].label);
			delete_key(&debug_state.vaos, vao);
		}
	}
}

delete_vertex_array :: proc (vao : Vao_id, loc := #caller_location) {
	vao := vao;
...
associate_buffer_with_vao :: proc (vao : Vao_id, buffer : Buffer_id, attributes : []Attribute_info_ex, #any_int divisor : u32 = 0, loc := #caller_location) {
	...
	if cpu_state.gl_version >= .opengl_4_5 {
        ...
        for attrib, i in attributes {
            ...
            gl.VertexArrayAttribFormat(auto_cast vao, auto_cast attrib.location, auto_cast get_attribute_type_dimensions(attrib.attribute_type), auto_cast get_attribute_primary_type(attrib.attribute_type), attrib.normalized, auto_cast attrib.offset);
            gl.EnableVertexArrayAttrib(auto_cast vao, auto_cast attrib.location);
            gl.VertexArrayVertexBuffer(auto_cast vao, auto_cast attrib.location, auto_cast buffer, 0, attrib.stride);
            
            if divisor != 0 {
                gl.VertexArrayBindingDivisor(auto_cast vao, auto_cast attrib.location, divisor);
            }
        }
...
associate_index_buffer_with_vao :: proc(vao : Vao_id, buffer : Buffer_id) {
	...
	if cpu_state.gl_version >= .opengl_4_5 {
		// Directly associate the index buffer with the VAO
		gl.VertexArrayElementBuffer(auto_cast vao, auto_cast buffer);  // Directly bind the element array buffer to the VAO
	} else {
		bind_vertex_array(auto_cast vao);
...
delete_buffers :: proc(location : Buffer_type, buffers : []Buffer_id, loc := #caller_location) {
	...
	when RENDER_DEBUG {
		for b in buffers {
			assert(b in debug_state.buffers, "cannot delete invalid buffer", loc);
			delete(debug_state.buffers[b].label);
			delete_key(&debug_state.buffers, b);
		}
	}
}

delete_buffer :: proc(location : Buffer_type, buffer : Buffer_id, loc := #caller_location) {
	...
	when RENDER_DEBUG {
		assert(buffer in debug_state.buffers, "cannot delete invalid buffer", loc);
		delete(debug_state.buffers[buffer].label);
...
discard_fence :: proc(fence : ^Fence, loc := #caller_location){
	...
	when RENDER_DEBUG {
		...
		delete(debug_state.syncs[fence.sync].label);
	}
...
@(require_results)
make_resource :: proc(bytes_count : int, buffer_type : Buffer_type, resource_usage : Resource_usage, data : []u8, label := "", loc :=#caller_location) -> Resource {
	...
	return make_resource_desc(resource_desc, data, label, loc);
}

@(require_results)
make_resource_desc :: proc(desc : Resource_desc, data : []u8, label := "", loc := #caller_location) -> Resource {
	...
	resource.buffer = gen_buffer(resource.buffer_type, label, loc = loc);
...
//uses temp allocator by default
get_buffer_label :: proc (buf : Buffer_id, alloc := context.temp_allocator, loc := #caller_location) -> string {
	context.allocator = alloc;
	
	when RENDER_DEBUG {
		assert(buf in debug_state.buffers, "invalid buffer", loc)
		return debug_state.buffers[buf].label;
	}
...
@(require_results)
gen_render_buffer :: proc (label : string, loc := #caller_location) -> Rbo_id {
...
delete_render_buffers :: proc(rbos : []Rbo_id, loc := #caller_location) {
	...
	when RENDER_DEBUG {
		for r in rbos {
			assert(r in debug_state.rbos, "Cannot delete invalid render buffer", loc);
			delete(debug_state.rbos[r].label);
...
delete_render_buffer :: proc(rbo : Rbo_id, loc := #caller_location) {
	...
	when RENDER_DEBUG {
		assert(rbo in debug_state.rbos, "Cannot delete invalid render buffer", loc);
		delete(debug_state.rbos[rbo].label);
		delete_key(&debug_state.rbos, rbo);
	}
}

gen_frame_buffer :: proc (label : string, loc := #caller_location) -> Fbo_id {
...
delete_frame_buffer :: proc(fbo : Fbo_id, loc := #caller_location) {
	...
	when RENDER_DEBUG {
		...
		delete(debug_state.fbos[fbo].label);
		delete_key(&debug_state.fbos, fbo);
	}
}

get_frame_buffer_label :: proc (fbo : Fbo_id, alloc := context.temp_allocator, loc := #caller_location) -> string {
	context.allocator = alloc;
	
	when RENDER_DEBUG {
		assert(fbo in debug_state.fbos, "invalid FBO", loc)
		return debug_state.fbos[fbo].label;
	}
...
active_texture_unit :: proc (slot : u32) {
	...
	cpu_state.active_unit = slot;
	
	if gpu_state.active_unit == slot {
		return;
	}

	gpu_state.active_unit = slot;
	gl.ActiveTexture(auto_cast gl.TEXTURE0 + auto_cast slot);
}

//// 1D textures ////
gen_texture1Ds :: proc (textures : []Tex1d_id, label : string, loc := #caller_location) {
...
@(require_results)
gen_texture1D :: proc (label : string, loc := #caller_location) -> (tex : Tex1d_id) {
...
delete_texture1D :: proc (texture : Tex1d_id, loc := #caller_location) {
	...
	//Unbind everywhere, so it can acctually be freed
	for t, i in gpu_state.bound_tex1ds {
		if t == texture {
			bind_texture1D(0, auto_cast i)
		}
	}
	...
	when RENDER_DEBUG {
		assert(texture in debug_state.tex1ds, "Cannot delete invalid texture1D", loc);
		delete(debug_state.tex1ds[texture].label);
		delete_key(&debug_state.tex1ds, texture);
	}
}

bind_texture1D :: proc(tex : Tex1d_id, slot : u32, loc := #caller_location) {
	
	cpu_state.bound_tex1ds[slot] = tex;
	
	if cpu_state.gl_version < .opengl_4_5 {
		active_texture_unit(slot);
	}

	if gpu_state.bound_tex1ds[slot] == tex {
		return;
	}
	
	gpu_state.bound_tex1ds[slot] = tex;
	if cpu_state.gl_version >= .opengl_4_5 {
		gl.BindTextureUnit(slot, auto_cast tex);
	} else {
		gl.BindTexture(.TEXTURE_1D, cast(gl.GLuint)tex);
	}
}

unbind_texture1D  :: proc(slot : u32) {
	when UNBIND_DEBUG {
		cpu_state.bound_tex1ds[slot] = 0;
		gpu_state.bound_tex1ds[slot] = 0;
		if cpu_state.gl_version >= .opengl_4_5 {
			gl.BindTextureUnit(auto_cast slot, 0);
		} else {
			active_texture_unit(slot);
...
//// 2D textures ////
gen_texture2Ds :: proc (textures : []Tex2d_id, label : string, loc := #caller_location) {
...
@(require_results)
gen_texture2D :: proc (label : string, loc := #caller_location) -> (tex : Tex2d_id) {
...
delete_texture2D :: proc (texture : Tex2d_id, loc := #caller_location) {	
	...
	//Unbind everywhere, so it can acctually be freed
	for t, i in gpu_state.bound_tex2ds {
		if t == texture {
			bind_texture2D(0, auto_cast i)
		}
	}
	...
	when RENDER_DEBUG {
		assert(texture in debug_state.tex2ds, "Cannot delete invalid texture2D", loc);
		delete(debug_state.tex2ds[texture].label);
		delete_key(&debug_state.tex2ds, texture);
	}
}

bind_texture2D :: proc(tex : Tex2d_id, slot : u32, loc := #caller_location) {
	
	cpu_state.bound_tex2ds[slot] = tex;
	
	if cpu_state.gl_version < .opengl_4_5 {
		active_texture_unit(slot);
	}

	if gpu_state.bound_tex2ds[slot] == tex {
		return;
	}
	
	gpu_state.bound_tex2ds[slot] = tex;
	if cpu_state.gl_version >= .opengl_4_5 {
		gl.BindTextureUnit(slot, auto_cast tex);
	} else {
...
unbind_texture2D  :: proc(slot : u32) {	
	when UNBIND_DEBUG {
		cpu_state.bound_tex2ds[slot] = 0;
		gpu_state.bound_tex2ds[slot] = 0;
		if cpu_state.gl_version >= .opengl_4_5 {
			gl.BindTextureUnit(auto_cast slot, 0);
		} else {
			active_texture_unit(slot)
...
clear_texture_1D :: proc (tex : Tex1d_id, clear_color : [4]f64, loc := #caller_location) {
	...
	if cpu_state.gl_version >= .opengl_4_5 { 
		...
		bind_texture1D(tex, 15); //TODO should we bind for the active texture? we could also always just use texture slot 0, but that might slower.
...
clear_texture_2D :: proc (tex : Tex2d_id, clear_color : [4]f64, loc := #caller_location) {
	...
	if cpu_state.gl_version >= .opengl_4_5 { 
		...
		bind_texture2D(tex, 15); //TODO should we bind for the active texture? we could also always just use texture slot 0, but that might slower.
...
clear_texture_3D :: proc (tex : Tex3d_id, clear_color : [4]f64, loc := #caller_location) {
	...
	if cpu_state.gl_version >= .opengl_4_5 { 
		...
		bind_texture3D(tex, 15); //TODO should we bind for the active texture? we could also always just use texture slot 0, but that might slower.
...
//// 3D textures ////
gen_texture3Ds :: proc (textures : []Tex3d_id, label : string, loc := #caller_location) {
...
@(require_results)
gen_texture3D :: proc (label : string, loc := #caller_location) -> (tex : Tex3d_id) {
...
delete_texture3D :: proc (texture : Tex3d_id, loc := #caller_location) {
	...
	//Unbind everywhere, so it can acctually be freed
	for t, i in gpu_state.bound_tex3ds {
		if t == texture {
			bind_texture3D(0, auto_cast i)
		}
	}
	...
	when RENDER_DEBUG {
		assert(texture in debug_state.tex3ds, "Cannot delete invalid texture3D", loc);
		delete(debug_state.tex3ds[texture].label);
		delete_key(&debug_state.tex3ds, texture);
	}
}

bind_texture3D :: proc(tex : Tex3d_id, slot : u32, loc := #caller_location) {

	cpu_state.bound_tex3ds[slot] = tex;
	
	if cpu_state.gl_version < .opengl_4_5 {
		active_texture_unit(slot);
	}

	if gpu_state.bound_tex3ds[slot] == tex {
		//return;
	}
	
	gpu_state.bound_tex3ds[slot] = tex;
	if cpu_state.gl_version >= .opengl_4_5 {
		gl.BindTextureUnit(slot, auto_cast tex);
	} else {
...
unbind_texture3D  :: proc(slot : i32) {
	
	when UNBIND_DEBUG {
		cpu_state.bound_tex3ds[slot] = 0;
		gpu_state.bound_tex3ds[slot] = 0;
		if cpu_state.gl_version >= .opengl_4_5 {
			gl.BindTextureUnit(auto_cast slot, 0);
		} else {
...
set_texture_border_color_3D :: proc (tex_id : Tex3d_id, border_color : [4]f32) {
	...
	if cpu_state.gl_version >= .opengl_4_5 {
		...
		gl.TexParameterfv(.TEXTURE_3D, .TEXTURE_BORDER_COLOR, &border_color[0]);
		unbind_texture3D(15);
	}
}
...
gen_texture_cubemap :: proc (label : string, loc := #caller_location) -> (tex : Tex_cubemap_id) {
	...
	// Create renderbuffer objects
	if cpu_state.gl_version >= .opengl_4_5 {
		gl.CreateTextures(.TEXTURE_CUBE_MAP, 1, auto_cast &tex);
	}
	else {
		gl.GenTextures(1, auto_cast &tex);
	}
	...
	when RENDER_DEBUG {
		debug_state.tex_cubemaps[tex] = {loc, strings.clone(label)};;
		if label != "" && cpu_state.gl_version >= .opengl_4_3 {
			if cpu_state.gl_version < .opengl_4_5 {
				bind_texture_cubemap(tex, 15);
				unbind_texture_cubemap(15);
			}
			clabel := fmt.ctprintf("Texture Cubemap %v: %v", tex, label);
			gl.ObjectLabel(.TEXTURE, auto_cast tex, auto_cast len(clabel), clabel);
		}
	}
...
delete_texture_cubemap :: proc (texture : Tex_cubemap_id, loc := #caller_location) {
	texture := texture;
	
	//Unbind everywhere, so it can acctually be freed
	for t, i in gpu_state.bound_tex_cube {
		if t == texture {
			bind_texture_cubemap(0, auto_cast i);
		}
	}
	...
	when RENDER_DEBUG {
		assert(texture in debug_state.tex_cubemaps, "Cannot delete invalid Cubemap Texture", loc);
		delete(debug_state.tex_cubemaps[texture].label);
		delete_key(&debug_state.tex_cubemaps, texture);
	}
}

bind_texture_cubemap :: proc (tex : Tex_cubemap_id, slot : u32, loc := #caller_location) {

	cpu_state.bound_tex_cube[slot] = tex;
	
	if cpu_state.gl_version < .opengl_4_5 {
		active_texture_unit(slot);
	}

	if gpu_state.bound_tex_cube[slot] == tex {
		return;
	}
	
	gpu_state.bound_tex_cube[slot] = tex;
	if cpu_state.gl_version >= .opengl_4_5 {
		gl.BindTextureUnit(slot, auto_cast tex);
	} else {
		gl.BindTexture(.TEXTURE_CUBE_MAP, auto_cast tex);
	}
}

unbind_texture_cubemap :: proc (slot : u32) {
		
	when UNBIND_DEBUG {
		cpu_state.bound_tex_cube[slot] = 0;
		gpu_state.bound_tex_cube[slot] = 0;
		if cpu_state.gl_version >= .opengl_4_5 {
			gl.BindTextureUnit(auto_cast slot, 0);
		} else {
			active_texture_unit(slot)
			gl.BindTexture(.TEXTURE_CUBE_MAP, 0);
		}
	}
	else {
		cpu_state.bound_tex_cube[slot] = 0;
...
set_texture_border_color_cubemap :: proc (tex_id : Tex_cubemap_id, border_color : [4]f32) {
	border_color := border_color;
	...
	if cpu_state.gl_version >= .opengl_4_5 {
		...
		bind_texture_cubemap(tex_id, 15);
		gl.TexParameterfv(.TEXTURE_CUBE_MAP, .TEXTURE_BORDER_COLOR, &border_color[0]);
		unbind_texture_cubemap(15);
	}
}

Cubemap_side :: enum u32 {
...
write_texure_data_cubemap :: proc (tex : Tex_cubemap_id, side : Cubemap_side, level, xoffset, yoffset : i32, width, height : gl.GLsizei, format : Pixel_format_upload, data : union {[]u8, Resource}, loc := #caller_location) {
	data_ptr : rawptr;
	...
	if cpu_state.gl_version >= .opengl_4_5 { 
		faces : [Cubemap_side]i32 = {.pos_x = 0, .neg_x = 1, .pos_y = 2, .neg_y = 3, .pos_z = 4, .neg_z = 5};
		//YES this is 3D they last coordinate is used as the cubemap face
		gl.TextureSubImage3D (cast(u32)tex, level, xoffset, yoffset, faces[side], width, height, 1, upload_format_gl_channel_format(format), upload_format_gl_type(format), data_ptr);
	}
	else {
		bind_texture_cubemap(tex, 15);
		gl.TexSubImage2D(auto_cast side, level, xoffset, yoffset, width, height, upload_format_gl_channel_format(format), upload_format_gl_type(format), data_ptr);
		unbind_texture_cubemap(15);
	}
}

//TODO look at glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS);

//This will for opengl 4.2 or below not generate mip maps, and they must be generated by generate_mip_maps_XD or the mipmap level must be set directly
setup_texure_cubemap :: proc (tex : Tex_cubemap_id, mipmaps : bool, width, height : gl.GLsizei, format : Pixel_format_internal, loc := #caller_location) {
	//type could be an option here, for now we only allow GL_UNSIGNED_BYTE as the type.
	levels : i32 = 1;
	...
	if cpu_state.gl_version >= .opengl_4_5 {
		...
		bind_texture_cubemap(tex, 15);
		gl.TexStorage2D(.TEXTURE_CUBE_MAP, levels, auto_cast format, width, height);
		unbind_texture_cubemap(15);
	}
	else {
		bind_texture_cubemap(tex, 15);
		for side in reflect.enum_field_values(Cubemap_side) {
			//TODO: .RGBA should match format, so for format=.R16, then it should be .R, not RGBA!
			gl.TexImage2D(auto_cast side, 0, auto_cast format, width, height, 0, internal_format_gl_channel_format(format), .UNSIGNED_BYTE, nil);
		}
		unbind_texture_cubemap(15);
	}
}

generate_mip_maps_cubemap :: proc (tex_id : Tex_cubemap_id) {
...
filtermode_texture_cubemap :: proc(tex_id : Tex_cubemap_id, mode : Filtermode, using_mipmaps : bool) {
	...
	if cpu_state.gl_version >= .opengl_4_5 { 
		...
		bind_texture_cubemap(tex_id, 15);
		gl.TexParameteri(.TEXTURE_CUBE_MAP, .TEXTURE_MIN_FILTER, cast(i32)min_mode);
		gl.TexParameteri(.TEXTURE_CUBE_MAP, .TEXTURE_MAG_FILTER, cast(i32)mag_mode);
		unbind_texture_cubemap(15);
	}
}
