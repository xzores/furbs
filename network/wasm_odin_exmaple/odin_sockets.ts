...
function odinSetupWebSockets(): WebAssembly.Imports {
	
	const socket_imports : WebAssembly.Imports = {
		odin_env: {

			//Takes in the place to could be "ws://localhost:1234" or "wss://localhost:1234"
			ws_create: (target_str_ptr: number, target_str_len: number, onconnect_str_ptr: number, onconnect_str_len: number, onrecv_str_ptr: number, onrecv_str_len: number,
						onerror_str_ptr: number, onerror_str_len: number, onclose_str_ptr: number, onclose_str_len:number
			): number => {
				
				if (!wasm || !memory) throw new Error("WASM not ready");	
				...
				socket.onmessage = (event) => {
					...
					if (typeof fn === "function") {
						const alloc = wasm.instance.exports.alloc as Function;
						const free = wasm.instance.exports.free as Function;
						if (!alloc) throw new Error("Odin does not export alloc");
						if (!free) throw new Error("Odin does not export free");
			...
			ws_send: (handle: number, kind: number, data_ptr: number, data_len: number) => {
