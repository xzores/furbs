...
sock_recv_fn :: #type proc "c" (sock : ws_socket_handle, kind : Recv_kind, data : [^]u8, length : int)
...
send :: proc (h : ws_socket_handle, data : []u8) {
	ws_send(h, raw_data(data), len(data));
}
...
foreign odin_env {
	...
	ws_send     :: proc(h : ws_socket_handle, data : rawptr, len : int) ---
